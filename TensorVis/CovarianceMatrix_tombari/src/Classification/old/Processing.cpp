...
bool Processing::fileRead()
{
 	...
 	else
	{
        ...
        if(_FileExtension == "las" || _FileExtension == "ply")
            scale();
...
void Processing::scale()
{
    ...
    minX = *std::min_element(tempData.begin(), tempData.end(), compare);
    maxX = *std::max_element(tempData.begin(), tempData.end(), compare);

    for(size_t i = 0; i < _inCloud->points.size(); i++)
        tempData[i] = _inCloud->points[i].y;

    minY = *std::min_element(tempData.begin(), tempData.end(), compare);
    maxY = *std::max_element(tempData.begin(), tempData.end(), compare);

    for(size_t i = 0; i < _inCloud->points.size(); i++)
        tempData[i] = _inCloud->points[i].z;

    minZ = *std::min_element(tempData.begin(), tempData.end(), compare);
    maxZ = *std::max_element(tempData.begin(), tempData.end(), compare);
...
bool Processing::structFeatClassification()
{
	...
	if(_probval.size() == 0 || pts_sz != _probval.size())
	{
        ...
        cout<<"number of transition points from critical to line-type features "<<l_s<<endl;
          cout<<"number of points from surface to line-type features "<<l_p<<endl;
