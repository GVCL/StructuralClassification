...
void tensor3dvoting::computeSaliencyVals(glyphVars &glyphtemp)
{
    ...
    float len = abs(glyphtemp.evals[2] + glyphtemp.evals[1] + glyphtemp.evals[0]);
...
bool tensor3dvoting::eigendecomposition(glyphVars &glyphtemp, size_t idx)
{
    ...
    glyphtemp.evals[0] = -1 * d[0] ;   //d[2] > d[1] > d[0]  -- after multiplication d[0] < d[1] < d[2]
    glyphtemp.evals[1] = -1 * d[1] ;
    glyphtemp.evals[2] = -1 * d[2] ;

    glyphtemp.evecs[0] = V[0][0];
    glyphtemp.evecs[1] = V[1][0] ;
    glyphtemp.evecs[2] = V[2][0];

    glyphtemp.evecs[3] = V[0][1];
    glyphtemp.evecs[4] = V[1][1] ;
    glyphtemp.evecs[5] = V[2][1];

    glyphtemp.evecs[6] = V[0][2];
    glyphtemp.evecs[7] = V[1][2] ;
    glyphtemp.evecs[8] = V[2][2];
