...
void graph::setSaliencyMaps(vector <probfeatnode> &probval)
{
    _probval = probval;
}
...
Return			:
*****************************************************************************************/
bool graph::r_search(std::vector<node> &seed, std::vector<node> &criticalSeed, IndxCont *neighborIndices, IndxCont *neighborSize, ftType radius, idxType rmaxpts, vector<myfloat3> &maxevecs)
{
...
Return      :
*****************************************************************************************/
int graph::check(vector<gnode>*graph, myfloat3 evecs, int index)
{
...
Return      :
*****************************************************************************************/
inline void graph::get_minidx(myfloat3 &u, myfloat3 &v, myfloat3 &evecs, vector<tempminval> *tempmin, idxType j)
{
    myfloat3 diff, w;
...
Return      :
*****************************************************************************************/
bool graph::propagate(std::vector<node> &seed, std::vector<node> &criticalSeed, idxType minIndex, idxType i, vector<gnode> *graph, vector<idxType> *graphIdx)
{
    nodeInfo tempnode;
    idxType m;
...
Return      :
*****************************************************************************************/
void graph::edgelinking(std::vector<node> &seed, std::vector<node> &criticalSeed, IndxCont &neighborSize, IndxCont &neighborIndices, vector<myfloat3> &maxevecs, vector<gnode> *graph, vector<idxType> *graphIdx)
{
...
//	cout<<"removedIndices size "<<removedIndices.size()<<endl;
    return;
}
...
void graph::r_clusterCriticalNode(std::vector<node> *criticalSeed, centroidType *centroid, float radius)
{
    ...
    vector< idxType > tempidx, tempcent;
    ...
    for(i = 0; i < criticalSeed->size(); i++)
    {
        if((*criticalSeed)[i].status == 0)
        {
            ...
            max_wt = _probval[(*criticalSeed)[i].idx].csclcp[0];
            ...
            {
                weight = _probval[(*criticalSeed)[tempidx[j]].idx].csclcp[0];
                if(weight > max_wt)
                    max_wt = weight;
            }
...
Return			:
*****************************************************************************************/
idxType graph::get_nearestCentroid(vector<idxType> &centroid, idxType idx, std::vector<node> *criticalSeed)
{
...
Return			:
*****************************************************************************************/
void graph::connectCentroidalNode(centroidType &centroid, idxType k, idxType i, vector<gnode> &graph, vector<idxType> &graphIdx)
{
...
Return			:
*****************************************************************************************/
bool graph::connectCriticalPoints(IndxCont &ng_sz, IndxCont &ng_idx, vector<gnode> &graph, vector<idxType> &graphIdx)
{
...
Return      :
*****************************************************************************************/
void graph::findDistinctElement(vector<idxType> *graphIdx)
{
    vector<idxType> tempa(graphIdx->size());
    vector<idxType>::iterator it;
...
Return      :
*****************************************************************************************/
void graph::constructGraph(vector<gnode> &graph, vector<idxType> &graphIdx)
{
    IndxCont neighborIndices, neighborSize;
    if(_node.size() == 0)
    {
